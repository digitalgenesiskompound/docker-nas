...
document.addEventListener('DOMContentLoaded', () => {
    // Retrieve the encryption passphrase from sessionStorage
    App.userPassphrase = sessionStorage.getItem('userPassphrase');
...
App.handleFileClick = function(file) {
    if (file.isDirectory) {
        // Navigate into directory
        App.navigationHistory.push(App.currentPath); // Push current path to history before navigating
        App.loadDirectory(file.path);
    } else {
        // Toggle selection
        App.toggleSelection(file.path);
    }
};
